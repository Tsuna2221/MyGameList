...
const Sidebar = () => {
    ...
    return (
        <div className='main-sidebar d-flex fdir-column txa-right'>
            <ul>
                {navItems.map(({items}) => (
                    <li className="nav-item mar-v-14 d-flex fdir-column align-end">
                        {items.map(({title, subitems}) => (
                                <Fragment>
                                    {subitems ? 
                                        <Fragment>
                                            <p className="c-light s-25 w-bold mar-t-8 mar-b-12">{title}</p>
                                            {subitems.map(({title}, index) => (
                                                <div onMouseLeave={() => setHover(null)} onMouseEnter={() => setHover(index)} className="sub-item d-flex flex-end pos-relative cw-fit">
